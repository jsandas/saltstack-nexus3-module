def test_loader_state_call_anonymous_access_test_mode(loaded_modules, loaded_states):
    ...
    loaded_modules["nexus3_anonymous_access.describe"] = (
        lambda: {"anonymous_access": {"enabled": False}}
    )
...
def test_loader_state_call_anonymous_access_apply_mode(loaded_modules, loaded_states):
    ...
    loaded_modules["nexus3_anonymous_access.enable"] = (
        lambda enabled: {"anonymous_access": {"enabled": enabled}}
    )
